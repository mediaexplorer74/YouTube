namespace LibVLCSharp.Platforms.UWP
{
    ...
    public class VideoView : UserControl, IVideoView
    {
        ...
        {
            // Ensure the panel has valid dimensions before creating the swap chain.
            // If not, defer creation until SizeChanged event.
            if (_panel.ActualWidth <= 0 || _panel.ActualHeight <= 0)
            {
                // Assuming SizeChanged will eventually be called and call CreateSwapChain again.
                // We might need a flag to prevent re-entry if SizeChanged fires rapidly
                // or if _loaded is set prematurely. For now, let Loaded handle the initial attempt.
                if (!_loaded) // Only try if not already marked as loaded
                {
                    return; // Wait for SizeChanged to fire
                }
            }
        ...
        void UpdateScale()
        {
            ...
            {
                M11 = 1.0f / _panel.CompositionScaleX,
                M22 = 1.0f / _panel.CompositionScaleY
            };
        }
